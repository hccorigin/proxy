...
# 1. PCRE : Supports regular expressions. Required by the NGINX Core and Rewrite modules
    && cd $NGINX_BUILD && wget https://github.com/PCRE2Project/pcre2/releases/download/pcre2-10.42/pcre2-10.42.tar.gz \
...
# 2. zlib : Supports header compression. Required by the NGINX Gzip module.
    && cd $NGINX_BUILD && wget http://zlib.net/zlib-1.3.tar.gz \
...
# 3. OpenSSL – Supports the HTTPS protocol. Required by the NGINX SSL module and others.
    && cd $NGINX_BUILD && wget https://github.com/openssl/openssl/releases/download/openssl-3.1.2/openssl-3.1.2.tar.gz \
...
# 4.LuaJIT2 : required by lua-nginx-module
    && cd $NGINX_BUILD && wget https://github.com/openresty/luajit2/archive/refs/tags/v2.1-20230410.tar.gz -O luajit2-2.1-20230410.tar.gz \
...
# 5. NDK : required by lua-nginx-module and more moredules
    && cd $NGINX_BUILD && wget https://github.com/vision5/ngx_devel_kit/archive/refs/tags/v0.3.2.tar.gz -O ngx_devel_kit-0.3.2.tar.gz \
...
###- Nginx 호환버전 확인 : https://github.com/openresty/lua-nginx-module#nginx-compatibility 
    && cd $NGINX_BUILD && wget https://github.com/openresty/lua-nginx-module/archive/refs/tags/v${NGINX_LUA_MOD_VER}.tar.gz -O lua-nginx-module-${NGINX_LUA_MOD_VER}.tar.gz \
    && tar zxf lua-nginx-module-${NGINX_LUA_MOD_VER}.tar.gz \

# 7. ngx_http_headers_more_filter_module : 
    && cd $NGINX_BUILD && wget https://github.com/openresty/headers-more-nginx-module/archive/refs/tags/v0.34.tar.gz -O headers-more-nginx-module-0.34.tar.gz \
    && tar zxf headers-more-nginx-module-0.34.tar.gz \

#######################################################################################
# 8. Compiling Dynamic modules of Nginx
    && cd $NGINX_BUILD && wget https://nginx.org/download/nginx-${NGINX_VERSION}.tar.gz \
...
# 9. OpenResty Official : required by lua-nginx-module
    && cd $NGINX_BUILD && wget https://openresty.org/download/openresty-1.21.4.2.tar.gz \
...
# 10. lua-resty-core & lua-resty-lrucache : required by lua-nginx-module
    && cd $NGINX_BUILD && wget https://github.com/openresty/lua-resty-core/archive/refs/tags/v0.1.27.tar.gz -O lua-resty-core-0.1.27.tar.gz \
